...
const long maxSteps = 1700;     // Maximale Steps für Roboter-Arm
...
void setup() {
  ...
  Serial.println("Maximale Steps: 1700 (digitale Endposition)");
...
void loop() {
  ...
  if (!endstopBeruehrt && endstopAktiv) {
    ...
    Serial.println("✅ Roboter-Arm bereit für Bewegung (0-1700 Steps)");
  }
...
void motorSteuern() {
  ...
  if (aktuelleZeit - letzterStepZeit >= stepIntervall) {
    ...
    if (richtungVorwaerts) {
      ...
      if (homingCompleted && stepZaehler >= maxSteps) {
        motorStoppen();
        Serial.println("🛑 MAXIMALE POSITION ERREICHT (1700 Steps)");
